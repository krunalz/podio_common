...
class Item(Area):
    ...
    def filter(self, app_id, attributes, **kwargs):
        ...
        attributes = json.dumps(attributes)
        return self.transport.POST(url="/item/app/%d/filter/" % app_id, body=attributes, type="application/json", **kwargs)
    ...
    def field_value(self, item_id, field_id):
        """
        return field value
        inputs: item_id, field_id
        """
    ...
    def create(self, app_id, attributes, silent=False, hook=True):
        ...
        return self.transport.POST(
            body=attributes,
            type="application/json",
            url="/item/app/%d/%s" % (app_id, self.get_options(silent=silent, hook=hook)),
        )
...
class Application(Area):
    ...
    def get_items_by_view(self, app_id, view_id, **kwargs):
        return self.transport.POST(url="/item/app/%s/filter/%s/" % (app_id, view_id), **kwargs)

    def filter_items(self, app_id, **kwargs):
        return self.transport.POST(url="/item/app/%s/filter/%s/" % app_id, **kwargs)
...
class Task(Area):
    ...
    def create_for(self, ref_type, ref_id, attributes, silent=False, hook=True):
        ...
        return self.transport.POST(
            body=attributes,
            type="application/json",
            url="/task/%s/%s/%s" % (ref_type, ref_id, self.get_options(silent=silent, hook=hook)),
        )
...
class Status(Area):
    ...
    def create(self, space_id, attributes):
        attributes = json.dumps(attributes)
        return self.transport.POST(url="/status/space/%s/" % space_id, body=attributes, type="application/json")
...
class View(Area):
    def create(self, app_id, attributes):
        ...
        if not isinstance(attributes, dict):
            raise TypeError("Must be of type dict")
        attributes = json.dumps(attributes)
        return self.transport.POST(url="/view/app/{}/".format(app_id), body=attributes, type="application/json")
    ...
    def update_view(self, view_id, attributes):
        ...
        if not isinstance(attributes, dict):
            raise TypeError("Must be of type dict")
        attribute_data = json.dumps(attributes)
        return self.transport.PUT(url="/view/{}".format(view_id), body=attribute_data, type="application/json")
...
class Comment(Area):
    def create(self, commentable_type, commentable_id, attributes):
        ...
        """
        attributes = json.dumps(attributes)
        return self.transport.POST(url="/comment/%s/%s/" % (commentable_type, commentable_id), body=attributes, type="application/json")
    ...
    def get_comment_by_id(self, comment_id):
        attributes = json.dumps(attributes)
        return self.transport.GET(url="/comment/%s/" % comment_id, type="application/json")


class Reminder(Area):
    def create(self, commentable_id, attributes):
        attributes = json.dumps(attributes)
        # /reminder/{ref_type}/{ref_id}
        return self.transport.PUT(url="/reminder/item/2469577331/", body=attributes, type="application/json")
        # return self.transport.PUT(url="/reminder/%s/%s/" % (commentable_type, commentable_id), body=attributes, type="application/json")
